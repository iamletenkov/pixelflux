...
/// `probe_codecs` on an open device.
pub(crate) fn probe_codecs_on(device: &Device) -> Result<Vec<Codec>, String> {
    let profiles = device.profiles()?;
    Ok(Codec::VIDEO
        .into_iter()
        .filter(|&codec| profile_ladder(codec, false).iter().any(|p| profiles.contains(p) && device.encode_entrypoint(*p).is_some()))
        .collect())
}
