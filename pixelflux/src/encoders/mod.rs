...
/// The software encoder this build runs for `codec`, or `None` for JPEG.
///
...
/// path and the full-frame software fallback under NVENC/VA-API both encode with.
...
pub fn select_frame_encoder(
    ...
    match software_session(settings, codec, rgba) {
        Ok(enc) => {
